...
namespace tImage {
    ...
    void merge_RGB24(Image* src_planes, Image* dst) {
		...
		for (t_int y = 0; y < height; y++) {
			...
			for (t_int x = 0; x < width; x++) {
				...
				dst_rowptr[x * 3] = src0_rowptr[x];
				dst_rowptr[x * 3 + 1] = src1_rowptr[x];
				dst_rowptr[x * 3 + 2] = src2_rowptr[x];
    ...
    void merge_generic(Image* src_planes, Image* dst) {

		const t_int width = dst->width();
        const t_int height = dst->height();
        const t_int channels = dst->channels();
		...
		for (t_int y = 0; y < height; y++) {
			...
			for (t_int x = 0; x < width; x++) {
                ...
                for (t_int c = 0; c < channels; c++) {
                    ...
                    dst_rowptr[x * channels + c] = src_planes[c].rowPtr(y)[x];
