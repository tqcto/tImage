...
namespace tImage {
    ...
    */
    ...
    void split_RGB24(Image* src, Image* dst_planes) {
        ...
        for (t_int y = 0; y < height; y++) {
            ...
            for (t_int x = 0; x < width; x++) {
                ...
                dst0_rowptr[x] = src_rowptr[x * 3];
                dst1_rowptr[x] = src_rowptr[x * 3 + 1];
                dst2_rowptr[x] = src_rowptr[x * 3 + 2];
    ...
    void split_generic(Image* src, Image* dst_planes) {

        const t_int width = src->width();
        const t_int height = src->height();
        const t_int channels = src->channels();
        ...
        for (t_int y = 0; y < height; y++) {
            ...
            for (t_int x = 0; x < width; x++) {
                ...
                for (t_int c = 0; c < channels; c++) {
                    ...
                    dst_planes[c].rowPtr(y)[x] = src_rowptr[x * channels + c];
